    ...
    for name, geo_object in obj_map.items():
        ...
        match type:
            ...
            case "Triangle":
                ...
                sympy_object = sympy.Triangle(sympy.Point2D(symbols[:2]), sympy.Point2D(symbols[2:4]), sympy.Point2D(symbols[4:6]))
            case "Circle":
                symbols = symbols = list(sympy.symbols(f"{name}_O_x {name}_O_y")) + [sympy.Symbol(f"{name}_radius", positive=True)] + [sympy.Symbol(f"{name}_radius", positive=True)]
    ...
    def plot(self, fig=None):
        ...
        for name, object in self.sympy_obj_map.items():
            if isinstance(object, sympy.Point):
                ...
                p0, p1 = object.points
                ax.plot([N(p0.x.subs(self.pairs)), N(p1.x.subs(self.pairs))], [N(p0.y.subs(self.pairs)), N(p1.y.subs(self.pairs))])
            elif isinstance(object, sympy.Triangle):
                p0, p1, p2 = object.vertices
                ax.plot([N(p0.x.subs(self.pairs)), N(p1.x.subs(self.pairs)), N(p2.x.subs(self.pairs)), N(p0.x.subs(self.pairs))], [N(p0.y.subs(self.pairs), p1.y.subs(self.pairs)), N(p2.y.subs(self.pairs)), N(p0.y.subs(self.pairs))])
