...
class Theorem:
    ...
    def read(path: Path) -> 'list[Theorem]':
        assert path.suffix == '.yml' or path.suffix == '.yaml', f'Theorems must be stored in YAML files! Tried to open {path}'
        ...
        for addr in THEOREM_FOLDER.rglob('*.yml'):
            ...
            for theorem in theorems:
                if theorem.name in filenames:
                    raise ProofParseError(f'Theorem {theorem.name} defined both in {addr} and in {filenames[theorem.name]}!')
