...
class EqualConstructionsLocus(LocusPattern):
    ...
    def match(self, object_: GeoObject, predicate: Predicate) -> Optional[ExtendedGeoObject]:
        ...
        if lhs.constructor.name != self.first_construction_name:
            lhs, rhs = rhs, lhs
        if (
            lhs.constructor.name != self.first_construction_name
            or rhs.constructor.name != self.second_construction_name
        ):
            return None
    ...
    def match_construction_index_options(
        self, object_: GeoObject, lhs: ConstructionObject, lhs_index: int, rhs: ConstructionObject, rhs_index: int
        ...
        if any(
            [object_ in component.involved_objects() for component in rest_of_lhs_components + rest_of_rhs_components]
        ):
            return None
