...
def point_on_circle(objects: Sequence[EmbeddedObject], circle: EmbeddedCircle) -> Tuple[EmbeddedPoint]:
    ...
    if len(points_on_circle) > 0:
        ...
        for i in range(len(angles) - 1):
            ...
            result_angles.extend([uniform(start, near_start), uniform(near_start, near_end), uniform(near_end, end)])
    else:
        result_angles.append(uniform(0, 2 * mp.pi))

    return tuple(
        [
            circle.center + EmbeddedPoint(mp.cos(angle), mp.sin(angle)).scale(circle.radius_squared**0.5)
...
def point_on_line(objects: Sequence[EmbeddedObject], line: EmbeddedLine) -> Tuple[EmbeddedPoint]:
    ...
    """
    points = [point for point in objects if isinstance(point, EmbeddedPoint)] + [
        project(circle.center, line)
        for circle in objects
        if isinstance(circle, EmbeddedCircle) and tangent(line, circle)
    ]

    points_on_line = [point for point in points if line.contains_point(point)]

    all_positions = [
        line.direction.scalar_product(point - line.point) / line.direction.length_squared() for point in points
    ]
    positions_on_line = [
        line.direction.scalar_product(point - line.point) / line.direction.length_squared() for point in points_on_line
    ]
