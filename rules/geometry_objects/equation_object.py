...
class EquationObject(GeoObject):
    ...
    def to_language_format(self) -> str:
        left_repr = (
            f'({self.left.to_language_format()})'
            if isinstance(self.left, EquationObject) and self.left.op.priority() < self.op.priority()
            else self.left.to_language_format()
        )
        right_repr = (
            f'({self.right.to_language_format()})'
            if isinstance(self.right, EquationObject) and self.right.op.priority() < self.op.priority()
            else self.right.to_language_format()
        )
        match self.op:
            case EqOp.ADD:
                if left_repr == '0':
