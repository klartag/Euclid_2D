...
def load_constructions_and_macros() -> None:
    ...
    global_constructions: dict[str, Construction] = {
        ...
        'log': LogConstruction('log', [log_arg_0], Symmetry.NONE, geo_object.GeoObject('res', SCALAR), [predicate_from_args('not_equals', (log_arg_0, geo_object.GeoObject('0', LITERAL)))], []),
    }
        ...
        if is_pred_file:
            ...
            for macro in macros:
                if macro.name in macro_filenames:
                    raise ProofParseError(f'Macto {macro.name} defined both in {filename} and in {macro_filenames[macro.name]}!')
            ...
            for construction in constructions:
                if construction.name in construction_filenames:
                    raise ProofParseError(f'Construction {construction.name} defined both in {filename} and in {construction_filenames[construction.name]}!')
...
def read_constructions(path: Path) -> list[Construction]:
    ...
    """
    assert path.suffix == '.yml' or path.suffix == '.yaml', f'Constructions must be stored in YAML files! Tried to open {path}'
    ...
    for construction_name, data in construction_data.items():
        ...
        for predicate_block in data.get(RESULT_PREDICATE_LABEL, []):
            match predicate_block:
                ...
                case str():
                    assert '=>' not in predicate_block, f'=> is not allowed in a conclusion statement. Did you mean to use it in a {POSS_CONCLUSIONS_LABEL} block?'
                    assert '<=' not in predicate_block, f'<= is not allowed in a conclusion statement. Did you mean to use it in a {POSS_CONCLUSIONS_LABEL} block?'
                    pred = parse_predicate(predicate_block, obj_map)
                    result_predicates.append(pred)
                case _:
                    raise NotImplementedError(f'Unknown predicate block type {type(predicate_block)} in construction {construction_name}')
        ...
        for flow in data.get(POSS_CONCLUSIONS_LABEL, []):
            ...
            left_preds = [predicate_map[part] if part in predicate_map else parse_predicate(part, obj_map) for part in left_parts]
            
            right_parts = [part.strip() for part in right.split('&')]
            right_preds = [predicate_map[part] if part in predicate_map else parse_predicate(part, obj_map) for part in right_parts]
            ...
            try:
                ...
                for predicate_block in data.get(RESULT_PREDICATE_LABEL, []) or []:
                    match predicate_block:
                        ...
                        case _:
                            raise ProofParseError(f'Unknown predicate block type {type(predicate_block)} in macro {macro_name}')
