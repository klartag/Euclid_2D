    ...
    * Make the EqualPattern store its own sub-patterns, and find only combinations using at least one new object.
    ...
    def update(self):
        ...
        new_objects = [
            ...
            or self.processed_objects[obj] != self.checker.equality_tracker.normalize(obj)
        ]
        ...
        for new_obj in new_objects:
            if isinstance(new_obj, ConstructionObject) and any(
                comp != self.geometry_tracker.equality_tracker.normalize(comp) for comp in new_obj.components
            ):
            ...
            new_repr = self.geometry_tracker.equality_tracker.normalize(new_obj)
