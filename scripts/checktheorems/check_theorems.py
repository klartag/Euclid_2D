...
def sample_predicates(type_obj, candidate_predicates=["between", "equals", "equals_mod_180", "equals_mod_360", "in", "between", "tangent", "parallel", "perpendicular", "collinear", "concyclic", "concurrent"], num_samples=3):
...
def check_compatibility(construction, type_obj_map, score_assignments = {"Point": 1, "Line": 2, "Triangle": 5, "Circle": 10, "Scalar": 1, "Angle": 1, "Orientation": 1}):
...
def check_theorem(theorem, idx, seed=100):
    ...
    for pred in theorem.required_predicates:
        if pred.name == "exists":
            ...
            predicates.append(parse_predicate(f"distinct({pred.components[0].name}, {pred.components[1].name})", theorem_obj_map))
        elif pred.name == "concyclic":
            new_circle = GeoObject(name="new_circle", type="Circle")
            ...
            new_point = GeoObject(name="new_point", type="Point")
            ...
            predicates.append(parse_predicate(f"identical(orientation({A}, {B}, {O}), orientation({B}, {C}, {O}), orientation({C}, {A}, {O}))", theorem_obj_map))
        else:
            predicates.append(pred)
    ...
    if good_losses.numel() > 0:
        ...
        for pred in result_predicates:
            if pred.name == "exists":
                ...
                final_result_predicates.append(parse_predicate(f"identical(orientation({A}, {B}, {O}), orientation({B}, {C}, {O}), orientation({C}, {A}, {O}))", theorem_obj_map))
            else:
